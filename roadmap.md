...
- [ ] **Test keys:** `zs_test_…` (Phase 1) — same as EmailFlare `eftest_`; sends never leave the dashboard.
...
- [ ] `zs_test_…` keys; sends are stored, never handed to `env.EMAIL`
- [ ] `email_logs` + `/mailbox` HTML/text preview
- [ ] REST: `POST /v1/emails` `{ from, to, subject, html?, text? }`
- [ ] Playground: send a test from the UI
...
- [ ] Live logs: recipient, from, API key prefix, Cloudflare message id, status, error
- [ ] Default from address in Settings (until Phase 4 domains)
...
- [ ] **Phase 0 + 1:** deploy, log in, mint a `zs_test_` key, send from another app or the playground, read it in `/mailbox`. No Cloudflare Email Sending yet.
- [ ] **Then Phase 2:** same flow with `zs_live_`, real inbox.
